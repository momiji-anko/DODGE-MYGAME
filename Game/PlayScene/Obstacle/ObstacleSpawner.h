#pragma once 
#include"../ISpawner.h"

class NormalObstacleSpawner :public ISpawner
{
public:
	NormalObstacleSpawner() = default;
	~NormalObstacleSpawner()override = default;
	bool Create(std::vector< std::unique_ptr<Actor>>&actors,
		const DirectX::SimpleMath::Vector3 & position,
		const float angle,
		IBehavior * behavior,
		DirectX::Model * model,
		DirectX::CommonStates * commonState
	) override;
};


class StickObstacleSpawner :public ISpawner
{
public:
	StickObstacleSpawner() = default;
	~StickObstacleSpawner()override = default;
	bool Create(std::vector< std::unique_ptr<Actor>>& actors,
		const DirectX::SimpleMath::Vector3& position,
		const float angle,
		IBehavior* behavior,
		DirectX::Model* model,
		DirectX::CommonStates* commonState
	) override;
};

//METEORITE
class MeteoriteObstacleSpawner :public ISpawner
{
public:
	MeteoriteObstacleSpawner() = default;
	~MeteoriteObstacleSpawner()override = default;
	bool Create(std::vector< std::unique_ptr<Actor>>& actors,
		const DirectX::SimpleMath::Vector3& position,
		const float angle,
		IBehavior* behavior,
		DirectX::Model* model,
		DirectX::CommonStates* commonState
	) override;
};

class RotateStickObstacleSpawner :public ISpawner
{
public:
	RotateStickObstacleSpawner() = default;
	~RotateStickObstacleSpawner()override = default;
	bool Create(std::vector< std::unique_ptr<Actor>>& actors,
		const DirectX::SimpleMath::Vector3& position,
		const float angle,
		IBehavior* behavior,
		DirectX::Model* model,
		DirectX::CommonStates* commonState
	) override;
};

class ReverseRotateStickObstacleSpawner :public ISpawner
{
public:
	ReverseRotateStickObstacleSpawner() = default;
	~ReverseRotateStickObstacleSpawner()override = default;
	bool Create(std::vector< std::unique_ptr<Actor>>& actors,
		const DirectX::SimpleMath::Vector3& position,
		const float angle,
		IBehavior* behavior,
		DirectX::Model* model,
		DirectX::CommonStates* commonState
	) override;
};

class MeanderingObstacleSpawner :public ISpawner
{
public:
	MeanderingObstacleSpawner() = default;
	~MeanderingObstacleSpawner()override = default;
	bool Create(std::vector< std::unique_ptr<Actor>>& actors,
		const DirectX::SimpleMath::Vector3& position,
		const float angle,
		IBehavior* behavior,
		DirectX::Model* model,
		DirectX::CommonStates* commonState
	) override;
};

//STRAIGHT
class StraightMoveRightObstacleSpawner :public ISpawner
{
public:
	StraightMoveRightObstacleSpawner() = default;
	~StraightMoveRightObstacleSpawner()override = default;
	bool Create(std::vector< std::unique_ptr<Actor>>& actors,
		const DirectX::SimpleMath::Vector3& position,
		const float angle,
		IBehavior* behavior,
		DirectX::Model* model,
		DirectX::CommonStates* commonState
	) override;
};

class StraightMoveLeftObstacleSpawner :public ISpawner
{
public:
	StraightMoveLeftObstacleSpawner() = default;
	~StraightMoveLeftObstacleSpawner()override = default;
	bool Create(std::vector< std::unique_ptr<Actor>>& actors,
		const DirectX::SimpleMath::Vector3& position,
		const float angle,
		IBehavior* behavior,
		DirectX::Model* model,
		DirectX::CommonStates* commonState
	) override;
};


class StraightMoveTopObstacleSpawner :public ISpawner
{
public:
	StraightMoveTopObstacleSpawner() = default;
	~StraightMoveTopObstacleSpawner()override = default;
	bool Create(std::vector< std::unique_ptr<Actor>>& actors,
		const DirectX::SimpleMath::Vector3& position,
		const float angle,
		IBehavior* behavior,
		DirectX::Model* model,
		DirectX::CommonStates* commonState
	) override;
};

class StraightMoveBottomObstacleSpawner :public ISpawner
{
public:
	StraightMoveBottomObstacleSpawner() = default;
	~StraightMoveBottomObstacleSpawner()override = default;
	bool Create(std::vector< std::unique_ptr<Actor>>& actors,
		const DirectX::SimpleMath::Vector3& position,
		const float angle,
		IBehavior* behavior,
		DirectX::Model* model,
		DirectX::CommonStates* commonState
	) override;
};

class BirdObstacleSpawner :public ISpawner
{
public:
	BirdObstacleSpawner() = default;
	~BirdObstacleSpawner()override = default;
	bool Create(std::vector< std::unique_ptr<Actor>>& actors,
		const DirectX::SimpleMath::Vector3& position,
		const float angle,
		IBehavior* behavior,
		DirectX::Model* model,
		DirectX::CommonStates* commonState
	) override;
};